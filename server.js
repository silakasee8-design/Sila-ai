...
function authRequired(req, res, next) {
  ...
  try { req.user = jwt.verify(readToken(req) || '', jwtSecret); next(); } catch { res.status(401).json({ error: 'Please log in to continue.' }); }
}
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try { const hash = await bcrypt.hash(password, 12); const result = await pool.query('INSERT INTO users(email,password_hash) VALUES($1,$2) RETURNING id,email', [email, hash]); setAuthCookie(res, tokenFor(result.rows[0])); res.status(201).json({ user: result.rows[0] }); }
  catch (error) { if (error.code === '23505') return res.status(409).json({ error: 'An account with that email already exists.' }); console.error(error); res.status(500).json({ error: 'Could not create account.' }); }
});
app.post('/api/auth/login', async (req, res) => {
  if (!pool || !jwtSecret) return res.status(503).json({ error: 'Authentication is not configured.' });
  const email = typeof req.body.email === 'string' ? req.body.email.trim().toLowerCase() : '';
  const password = typeof req.body.password === 'string' ? req.body.password : '';
  try { const result = await pool.query('SELECT id,email,password_hash FROM users WHERE email=$1', [email]); const user = result.rows[0]; if (!user || !(await bcrypt.compare(password, user.password_hash))) return res.status(401).json({ error: 'Invalid email or password.' }); setAuthCookie(res, tokenFor(user)); res.json({ user: { id: user.id, email: user.email } }); }
...
app.get('/api/history', authRequired, async (req, res) => { try { const result = await pool.query("SELECT role,content,created_at FROM messages WHERE user_id=$1 ORDER BY created_at DESC LIMIT 100", [req.user.id]); res.json({ messages: result.rows.reverse() }); } catch (error) { console.error(error); res.status(500).json({ error: 'Could not load chat history.' }); } });
app.delete('/api/history', authRequired, async (req, res) => { try { await pool.query('DELETE FROM messages WHERE user_id=$1', [req.user.id]); res.json({ ok: true }); } catch { res.status(500).json({ error: 'Could not clear chat history.' }); } });
...
app.post('/api/chat', authRequired, async (req, res) => {
  ...
  try {
    ...
    const completion = await client.chat.completions.create({ model: process.env.OPENAI_MODEL || 'gpt-4o-mini', messages: [{ role: 'system', content: 'You are SILA AI, a friendly and practical assistant. Help with writing, business ideas, creativity, productivity, and Swahili or English communication. Respond in the language the user uses.' }, ...previous.rows.reverse().map(row => ({ role: row.role, content: row.content }))], temperature: 0.7, max_tokens: 800 });
    ...
    res.json({ reply });
  } catch (error) { console.error('Request failed:', error.message); res.status(502).json({ error: 'The AI service could not process your request.' }); }
});

app.get('*', (req, res) => res.sendFile(path.join(__dirname, 'index.html')));
initDb().then(() => app.listen(port, () => console.log(`SILA AI is running at http://localhost:${port}`))).catch(error => { console.error('Database startup failed:', error); process.exit(1); });
